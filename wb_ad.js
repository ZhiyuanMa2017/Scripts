...
const path31 = "/statuses/repost_timeline";
...
if (
    ...
    if (obj.head_cards) obj.head_cards = null;
    body = JSON.stringify(obj);
} else if (url.indexOf(path4) !== -1) {
    let obj = JSON.parse(body);
    let delType = ['广告', '推荐', '热推'];
    let items = obj.datas || [];
    if (items.length !== 0) {
        let newItems = [];
...
function filter_messageflow_notice(data) {
    ...
    for (let index = items.length-1; index >= 0; index--) {
        const item = items[index];
        if (item["isrecommend"] === true || item.msg_card?.ad_tag) {
            items.splice(index,1)
        }
    }
}
